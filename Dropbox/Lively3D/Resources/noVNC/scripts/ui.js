...
/*    html += '<div id="VNC_controls">';
  ...
  //  WebUtil.selectStylesheet(UI.getSetting('stylesheet'));
    ...
    UI.initSetting('host', '130.230.5.168');
    UI.initSetting('port', '8787');
	...
	Util.Debug("noVNC: Settings loaded");

    UI.rfb = RFB({'target': $D('VNC_canvas'),
                  'onUpdateState': UI.updateState,
                  'onClipboard': UI.clipReceive});

	Util.Debug("Unfocus clipboard when over the VNC area");
    // Unfocus clipboard when over the VNC area
	/*
    $D('VNC_canvas').onmousemove = function () {
...
updateState: function(rfb, state, oldstate, msg) {
	...
	Util.Debug("UpdateState function");
    switch (state) {
        case 'failed':
			Util.Debug("UpdateState function: Failed");
        case 'fatal':
   //         c.disabled = true;
   //         cad.disabled = true;
   //         UI.settingsDisabled(true, rfb);
			Util.Debug("UpdateState function: Fatal");
   ...
   ////         cad.disabled = false;
            ...
            break;
        case 'disconnected':
			Util.Debug("UpdateState function: Disconnected");
        case 'loaded':
			Util.Debug("UpdateState function: loaded");
            //c.value = "Connect";
            ...
            break;
        case 'password':
			Util.Debug("UpdateState function: Password");
  ...
  ////          c.disabled = false;
   //         cad.disabled = true;
            ...
            break;
        default:
			Util.Debug("UpdateState function: Default");
   //         c.disabled = true;
   //         cad.disabled = true;
            UI.settingsDisabled(true, rfb);
            klass = "VNC_status_warn";
...
connect: function() {
    ...
    host = "130.230.5.168"; // $D('VNC_host').value;
    port = "8787"; // $D('VNC_port').value;
    password = "123456";
    if ((!host) || (!port)) {
...
setLively3DMouse: function(coord) {
